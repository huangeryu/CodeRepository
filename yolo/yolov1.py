...
class Yolo(object):
    ...
    def _build_net(self):
        ...
        net = self._conv_layer(self.images, 1, 64, 7, 2)#7*7*64-s-2
        net = self._maxpool_layer(net, 1, 2, 2)
        net = self._conv_layer(net, 2, 192, 3, 1)#3*3*192-s-1
        net = self._maxpool_layer(net, 2, 2, 2)
        net = self._conv_layer(net, 3, 128, 1, 1)#1*1*128-s-1
    ...
    def _build_detector(self):
        ...
        box_classes = tf.argmax(scores, axis=1)#下标
        box_class_scores = tf.reduce_max(scores, axis=1)#分数

        # filter the boxes by the score threshold
        filter_mask = box_class_scores >= self.threshold
        scores = tf.boolean_mask(box_class_scores, filter_mask)#[S*S*B,C]
        boxes = tf.boolean_mask(boxes, filter_mask)#[S*S*B,4]获得置信度最大的窗口
        box_classes = tf.boolean_mask(box_classes, filter_mask)#获得置信度最大的类的下标
